  ...
  function renderBoards() {
    ...
    BOARDS.forEach(b => {
      ...
      t.onclick = (e) => {
        ...
        if (window.__clampDD) __clampDD(dd);   // 窄屏防右溢出
      };
  ...
  function renderJinguiFangji(s) {
    ...
    getJSON("/api/renji/jingui/chapters").then(d => {
      ...
      chapters.forEach((ch, idx) => {
        const b = el("button", "filter-tab" + (idx === 0 ? " active" : ""),
