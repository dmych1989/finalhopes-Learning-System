...
(function () {
  ...
  .point-card{border:1px solid #2a5;background:#0f2a40;border-radius:8px;padding:10px;margin:8px 0;color:#dfeefb}
  .point-card h4{color:#ffd479;margin:0 0 6px}
  .point-card .sec{margin:4px 0;color:#dfeefb;line-height:1.85}
  .point-card .sec b{color:#9fc1da}
  .point-card .nishi-box .sec{color:#e6f2fb}
  ...
  // 转义 + 保留换行：源数据（处方/治法/讲义）多为多行文本，需保持分段
  ...
  function showPoint(p) {
    ...
    ["治疗症状", "取穴位置", "针刺方法"].forEach(k => {
      if (blocks[k]) h += "<div class='sec'><b>" + k + "：</b><br>" + nl2br(blocks[k].trim()) + "</div>";
    });
    const nishi = p.nishi || {};
    const nk = Object.keys(nishi).filter(k => nishi[k] && nishi[k].trim());
    if (nk.length) {
      h += "<div class='nishi-box'><b style='color:#ffd479'>倪师穴位详解</b>";
      nk.forEach(k => h += "<div class='sec'><b>" + esc(k) + "</b><br>" + nl2br(nishi[k]) + "</div>");
      h += "</div>";
    }
  ...
  function renderFields(s) {
    filterBar.innerHTML = "";
    // size 放大到 2000：服务端默认 size=60 会截断（如百病针灸治疗 206 条 / 针灸医案 220 条）
    getJSON("/api/renji/list?sub=" + s.src + "&size=2000").then(d => {
      const items = d.items || [];
      filterBar.innerHTML = "<div class='hint' style='padding:4px'>共 " + items.length + " 条</div>";
      ...
      items.forEach(it => {
        ...
        li.onclick = () => {
          ...
          getJSON("/api/renji/item?sub=" + s.src + "&i=" + it.i).then(rec => {
            ...
            const f = rec.fields || {};
            Object.keys(f).forEach(k => { if (f[k]) h += "<div class='sec'><b>" + esc(k) + "：</b><br>" + nl2br(f[k]) + "</div>"; });
            h += "</div>";
            detailPane.innerHTML = h;
            detailPane.scrollTop = 0;
          }).catch(() => { detailPane.innerHTML = "<div class='hint'>详情加载失败，请重试。</div>"; });
        };
        resultList.appendChild(li);
      });
    }).catch(() => { detailPane.innerHTML = "<div class='hint'>列表加载失败，请重试。</div>"; });
  }
  ...
  // ---------- 金匮要略（人纪经方） ----------
  // 「方剂详解」采用三级布局：左侧篇目（一级）+ 中间方剂（二级）+ 右侧详解；
  // 其余 4 项（百病针灸治疗 / 辨症针灸治疗 / 脏腑辨证论治 / 症状辨证取穴）复用
  // 全局 fields 渲染（中间列表 + 右侧详情）。样式全部复用本页既有类。
  function hideCatNav() {
  ...
  function renderJinguiFangji(s) {
    ...
    getJSON("/api/renji/jingui/chapters").then(d => {
      ...
      if (nav) {
        ...
        nav.innerHTML = "";
        nav.appendChild(el("div", "catnav-title", "金匮要略 · 篇目"));
        chapters.forEach((ch, idx) => {
          const b = el("button", "catnav-item" + (idx === 0 ? " active" : ""));
          b.innerHTML = "<span class='cn-label'>" + esc(ch.short || ch.name) + "</span>" +
            "<em class='cn-count'>" + ch.count + "</em>";
          b.title = "第" + ch.no + "篇 " + ch.name;
          b.onclick = () => {
            [...nav.querySelectorAll(".catnav-item")].forEach(x => x.classList.remove("active"));
